...
void ImGuiReloadFonts() {
    ...
    if(FontTexture.id != 0)  UnloadTexture(FontTexture);
...
bool ImGuiInit(bool dark) {
    igCreateContext(NULL);

    if(dark) igStyleColorsDark(igGetStyle());
    else igStyleColorsLight(igGetStyle());
...
static void ImGuiUpdateMouseCursor(void) {
    ImGuiIO* io = igGetIO();
...
static void ImGuiUpdateMousePosAndButtons(void) {
    ImGuiIO* io = igGetIO();

    // Set OS mouse position if requested (rarely used, only when ImGuiConfigFlags_NavEnableSetMousePos is enabled by user)
    if (io->WantSetMousePos) SetMousePosition(io->MousePos.x, io->MousePos.y);
...
void ImGuiNewFrame(void) {
    ...
    if (GetMouseWheelMove() > 0) io->MouseWheel += 1;
    else if (GetMouseWheelMove() < 0) io->MouseWheel -= 1;
}
...
void DrawTriangleVertex(ImDrawVert idx_vert)
{
    Color *c; c = (Color *)&idx_vert.col;
    rlColor4ub(c->r, c->g, c->b, c->a);
    rlTexCoord2f(idx_vert.uv.x, idx_vert.uv.y);
    rlVertex2f(idx_vert.pos.x, idx_vert.pos.y);
}

void ImGuiRenderDrawTriangles(unsigned int count, const ImDrawIdx *idx_buffer, const ImDrawVert *idx_vert, unsigned int texture_id) {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd->UserCallback) {
                ...
                BeginScissorMode(rectX, rectY, rectW, rectH);
                {
                    ...
                    ImGuiRenderDrawTriangles(pcmd->ElemCount, idx_buffer, vtx_buffer, *ti);
                }
            }
